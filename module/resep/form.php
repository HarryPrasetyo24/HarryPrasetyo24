<?php
	...
	$url = "my_profile.php?page=my_profile&module=resep&action=form";

	$queryKonfirmasiresep = mysqli_query($koneksi, "SELECT * FROM konfirmasi_pem_resep ORDER BY resep_id DESC LIMIT $mulai_dari, $data_per_halaman");
	
	if(mysqli_num_rows($queryKonfirmasiresep) == 0){
		...
		echo "<tr class='baris-title'>
			...
			$no=1;
			while($row=mysqli_fetch_assoc($queryKonfirmasiresep)){
		...
		echo "</table>";
		$queryHitungKategori = mysqli_query($koneksi, "SELECT * FROM konfirmasi_pem_resep");
